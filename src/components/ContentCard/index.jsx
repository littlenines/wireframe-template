...
const ContentCard = (props) => {
  const { setClass, videoSrc='', imageSrc='', title, description=[], link, linkTitle='Read More' } = props;
  return (
    <div className={`${style.content_card} ${setClass ?? ''}`} {...props}>
      <div className={`${style.media} ${style.video_poster}`}>
        {videoSrc && <video controls><source src={videoSrc} type="video/mp4" /></video>}
        {imageSrc && <img src={imageSrc} alt="card image" />}
      </div>
      <div className={style.content}>
        <h3 className='card_heading_typography'>{title}</h3>
        <div className={style.description}>
          {description.length > 0 && description.map((item, index) => <p key={index} className='card_text_typography'>{item}</p>)}
        </div>
      ...
      <div className={style.content_card_footer}>
        ...
        <div className={style.actions}>
          <div className={style.actions_comments}>
            <img src="icons/message-circle.svg" width={24} height={24} alt="comments" />
            <p className='card_text_typography'>123</p>
          </div>
          <div className={style.actions_views}>
            <img src="icons/eye.svg" width={24} height={24} alt="view" />
